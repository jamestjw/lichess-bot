...
def logging_configurer(
    level: int, filename: str | None, disable_auto_logs: bool
) -> None:
    """
...
def start(
    ...
    control_stream = multiprocessing.Process(
        target=watch_control_stream, args=(control_queue, logging_queue, li)
    )
    ...
    pgn_listener = multiprocessing.Process(
        target=write_pgn_records,
        args=(pgn_queue, logging_queue, config, user_profile["username"]),
    )
    pgn_listener.start()
...
def handle_challenge(
    ...
    opponent_engagements = Counter(
        game["opponent"]["username"] for game in active_games
    )
    opponent_engagements.update(
        challenge.challenger.name for challenge in challenge_queue
    )
...
def play_game(
    ...
    with li.get_game_stream(game_id) as response:
        ...
        game = model.Game(
            initial_state, user_profile["username"], li.baseUrl, abort_time
        )
